...
class TestResolverMatches:
    ...
    def test_from_dump(
        ...
        foo_source = source_factory(name="foo").source
        bar_source = source_factory(name="bar").source

        original = ResolverMatches(
            sources=[foo, bar], query_results=[foo_query_data, bar_query_data]
        )

        new = ResolverMatches.from_dump(
            cluster_key_map=original.as_dump(), sources=[foo_source, bar_source]
        )

        assert new.sources[0] is foo_source
        assert new.sources[1] is bar_source
