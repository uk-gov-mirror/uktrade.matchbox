...
follows from that: the plan has to be put somewhere findable, and the per-step detail
has to quote the position.

**Something always carries the key, and only one thing does.** A collection puts the
plan in exactly one place — whichever place you are looking at:

* **A live tree**, at a terminal or in a notebook: one frame, redrawn in place, so a
  fifty-step plan doesn't scroll the shell away. The tree *is* the key and the progress
  at once, because the row that lights up is the row numbered `[7]`. It stays on screen
  throughout and is left there in full when the run ends, so a `[step 7]` in the log
  beside it always has something to resolve against.
* **A logged tree**, when nothing is drawn — CI, a job runner, a redirected stream. One
  multi-line record, the way a traceback is, so it can't be interleaved apart.

Never both: a drawn tree already answers what a logged one would, and printing the plan
twice into the same screen is just noise.

The per-step records are logged either way, each prefixed with its position. They land
on the `matchlab` logger and are silent unless you have configured logging.

Which of the two you get is the `interactive` argument to `Step.collect`, and that
argument is honoured. It is named for the assumption it makes rather than the widget it
produces: drawing means *someone is watching*, so the plan can be a thing on screen
that the session throws away. `interactive=False` is what puts the tree in the log,
and is what a run whose output outlives the session wants.

**The two share a terminal without being told how to.** A handler holding a raw stream
would write straight past Rich and land in the middle of the frame being redrawn, and
that describes the handler `logging.basicConfig()` builds — the one nearly every caller
has. So a report borrows those handlers for as long as it owns the terminal and points
them at the console, putting them back afterwards: see
`matchlab.core.logging.through_console`. Records interleave above the frame instead of
through it, and no one has to know that in advance. `ConsoleHandler` is the same thing
arranged up front, for a handler you are building yourself.

A plan taller than the terminal is **windowed, not abandoned**. Rich would crop a live
region to its first rows (`rich.live_render`), which hides the very step you are
waiting on, so the frame picks its own slice instead: the rows around whichever step is
running, captioned with how much sits either side. The window follows the run down the
tree, and the last frame — nothing left to redraw in place — is the tree in full.

So the terminal decides how much of the frame you see, and nothing else. It has no say
in whether a tree is drawn, nor in what the per-step records say.
...
class Progress:
    ...
    One object owns everything a running collection does besides the work itself: what
    is drawn, what is logged, which position a record is attributed to, and — while a
    frame is up — where the application's own handlers write. They are together because
    they have to agree. A frame and a log disagreeing about which step is running would
    ...
    def __init__(
        ...
        *,
        ...
        #: was in force — an outer collection's step, if this one is nested — on `end`.
    ...
    def begin(self, step: Step) -> None:
        ...
        emits — a linker counting matches, a source reading the warehouse. None of
        that code can name its position, because a position belongs to the walk and
        not to the step; this is the walk, so it is the one that can say.
        """
        self._release()
        self._running = step
    ...
    def __enter__(self) -> Progress:
        ...
        try:
            if self._live is not None:
                ...
                # The tree once, up front: the records that follow quote positions, and
                # this is what says which node each position is. A drawn tree is already
                # that key — on screen throughout, and left in full when the run ends —
                # so logging a second copy beside it would only print the plan twice.
    ...
    def __exit__(
        ...
        try:
            ...
            # Even if teardown failed. A report that never released the flag would
            # leave every later collection believing it was nested, and undrawn.
            if not self._nested:
                _REPORTING = False
        if exc_type is None:
...
def report(root: Step, steps: Sequence[Step], interactive: bool | None) -> Progress:
    ...
    Args:
        ...
        interactive: Whether someone is watching — draw the plan rather than log it.
